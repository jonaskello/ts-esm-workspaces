...
function resolvePackageTargetString(packageResolve, target, subpath, match, packageJSONUrl, base, pattern, internal, conditions) {
...
function resolvePackageTarget(packageResolve, packageJSONUrl, target, subpath, packageSubpath, base, pattern, internal, conditions) {
    if (typeof target === "string") {
        return resolvePackageTargetString(packageResolve, target, subpath, packageSubpath, packageJSONUrl, base, pattern, internal, conditions);
    }
        ...
        for (let i = 0; i < target.length; i++) {
            ...
            let resolved;
            try {
                resolved = resolvePackageTarget(packageResolve, packageJSONUrl, targetItem, subpath, packageSubpath, base, pattern, internal, conditions);
            }
            catch (e) {
                lastException = e;
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            if (key === "default" || conditions.has(key)) {
                ...
                const resolved = resolvePackageTarget(packageResolve, packageJSONUrl, conditionalTarget, subpath, packageSubpath, base, pattern, internal, conditions);
                if (resolved === undefined)
                    continue;
                return resolved;
            }
...
function packageExportsResolve(packageResolve, packageJSONUrl, packageSubpath, packageConfig, base, conditions) {
    ...
    if (ObjectPrototypeHasOwnProperty(exports, packageSubpath) &&
        ...
        const resolved = resolvePackageTarget(packageResolve, packageJSONUrl, target, "", packageSubpath, base, false, false, conditions);
    ...
    if (bestMatch) {
        ...
        const resolved = resolvePackageTarget(packageResolve, packageJSONUrl, target, bestMatchSubpath, bestMatch, base, pattern, false, conditions);
        if (resolved === null || resolved === undefined)
            throwExportsNotFound(packageSubpath, packageJSONUrl, base);
        if (!pattern)
    ...
    if (packageConfig.exists) {
        ...
        if (imports) {
            if (ObjectPrototypeHasOwnProperty(imports, name) &&
                ...
                const resolved = resolvePackageTarget(packageResolve, packageJSONUrl, imports[name], "", name, base, false, true, conditions);
                if (resolved !== null)
                    return { resolved, exact: true };
            }
            else {
                ...
                if (bestMatch) {
                    ...
                    const pattern = StringPrototypeIncludes(bestMatch, "*");
                    const resolved = resolvePackageTarget(packageResolve, packageJSONUrl, target, bestMatchSubpath, bestMatch, base, pattern, true, conditions);
                    if (resolved !== null) {
                        if (!pattern)
